...
\textbf{Note}:
\begin{itemize}
    \item All eigenvalues of a \textit{pd} matrix are positive
    \item By spectral decomposition, any \textit{pd} matrix can be written as 
    \[
      A = P \Lambda P^T, 
    \]
    ...
    \]
    \item $A^{-1}$ exists and is given by 
    \[
      A^{-1} = P \Lambda^{-1} P^T
    \]
    Proof:
    ...
    \]
    \item $A$ has a square root. Given a \textit{pd} matrix $A$, we say that $B$ is the square root of $A$ if $BB = A$ 
    \[
    B = P \begin{bmatrix} 
    ...
    \end{bmatrix} P^T
    \]
    \item Sum of eigenvalues is the trace of $A$
    \[
      \sum\limits_{i=1}^{n}  \lambda_i = tr(A)
    \]
\end{itemize} 

\textbf{Note}: What does it mean to assume that the covariance matrix is \textit{pd}? \\
...
\section{Properties of the multivariate normal}
...
then we say that $ \vtr{X} \perp \vtr{Y}$ if and only if $\mtr{\Sigma_{11}} = \mtr{\Sigma_{21}}^T = \mtr{0}$. 

\textbf{Proof}: The forward direction is obvious. \\
...
\begin{itemize}
    ...
    \item However, if $(X, Y)$ are jointly normal, then 
    \[
