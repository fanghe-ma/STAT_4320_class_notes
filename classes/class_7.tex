...
\begin{framed}
    \textbf{Definition} (Consistency):  Suppose $ \hat{ \theta}_n$ is an estimate of $ \theta$ based on $n$ iid samples. Then, $ \hat{ \theta}_n$ is said to be consistent for $ \theta$ if 
    \[
...
\begin{framed}
    \textbf{Definition} (Method of Moments): Suppose $X_1, X_2, \hdots X_n$ iid from distribution $F$ which as $k$ unknown parameters $( \theta_1, \theta_2, \hdots \theta_k)$.  \\
...
\textbf{Ex} (Bernoulli): $X_1, \hdots X_n \sim Ber(p)$
\begin{align*}
...
\textbf{Ex} (Normal) $X_1, \hdots X_n \sim N(\mu, \sigma^2)$ 
...
\subsection*{Estimation of Parameters of GMM}
...
\subsection*{Pearson's Sixth-Moment Test}
...
\subsection*{Theorem (Kalai, Moitra, Valiant, 2010)}
