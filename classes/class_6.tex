...
\subsection{Basic Framework of Statistical Estimation}

Given iid samples $X_1, X_2, \hdots X_n$, how do we infer / estimate parameters of $F$?

\begin{example}
(Bernoulli): Given $X_1, X_2, \hdots, X_n$ iid $Ber(p)$, 
\[
    X_i = \begin{cases}
...
\begin{example}
(Normal): $X_1, X_2, \hdots X_n$ iid $N(\mu, \sigma^2)$. Estimates for parameters are 
\[
...
\begin{definition}
    (Unbiasedness):  An estimate $\hat{ \theta}$ is said to be unbiased for a parameter $ \theta$ if 
    \[
...
\begin{definition}
    (Variance of an estimate): The variance of an estimate $ \hat{ \theta}$ is 
    \[
...
\begin{corollary}
If $ \hat{ \theta}$ unbiased, then $MSE( \hat{ \theta}) = Var( \hat{ \theta})$
\end{corollary}
