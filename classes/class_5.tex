...
\begin{definition}
The $\chi_{d}^2$ is the sum of $d$ iid standard normals squared. 
\[
...
\subsection{Sample Variance, cont'd}

\begin{result}
The sample variance follows a \textit{chi-squared} distribution. 
\[
...
\begin{proof}
  Let
  \[
  \vtr{Y} = \begin{pmatrix} Y_1 \\ Y_2 \\ \vdots \\ Y_n \end{pmatrix}    \begin{pmatrix} X_1 - \bar{X}  \\ X_2 - \bar{X}  \\\vdots \\ X_n - \bar{X} \end{pmatrix}   
  \]
  
We express $\vtr{Y} = \mtr{A} \vtr{X}$ for some matrix $\mtr{A}$.
\[
\vtr{Y} = \begin{pmatrix} 
...
\subsection{Joint distribution of sample mean and sample variance}
...
\begin{result}
If $X_1, X_2, \hdots X_n$ iid normal, $\bar{X} , s^2$ independent. 
\end{result}

\begin{proof}
Define 
\[
\vtr{Z} = \begin{pmatrix} X_1 - \bar{X} \\ X_2 - \bar{X} \\ \vdots \\ X_n - \bar{X} \\ \bar{X}  \end{pmatrix} 
\]
...
\begin{remark}
    As a general strategy, to show independence, we can show in two steps
    \begin{enumerate}
